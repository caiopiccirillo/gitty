    ...
    if app.diff.is_empty() {
        ...
        let lines: Vec<Line> = app
            ...
            .map(|line| {
                ...
                Line::styled(text, style_for(line.kind))
            })
            .collect();
...
fn status_bar(app: &App) -> Paragraph<'static> {
    ...
    let line = Line::from(vec![
        Span::styled(
            format!(" {} file(s) · line {position} ", app.diff.file_count),
            Style::new().fg(Color::Black).bg(Color::Gray),
        ),
        Span::styled(
            " q quit · ↑/k ↓/j scroll · PgUp/PgDn page · g/G top/bottom ",
...
mod tests {
    ...
    fn renders_prefixed_lines_and_status_bar() {
        ...
        assert!(screen.contains("1 file(s) · line 1/3"));
    }
